    ...
    <tbody>
        ...
        ?>
            <tr>
                ...
                <!-- AI SUMMARY -->
                <td>
                    <button class="btn btn-sm btn-warning btn-ai"
                            data-id="<?= $row['id'] ?>"
                            data-isi="<?= htmlspecialchars($row['isi'], ENT_QUOTES) ?>">
                        ✨ Summarize
                    </button>
                    
                    <!-- 🔥 MODIFIKASI: Tampilkan summary yang sudah ada -->
                    <div id="summary-<?= $row['id'] ?>" class="mt-2 small">
                        <?php if(!empty($row["summary"])): ?>
                            <strong>Summary:</strong><br>
                            <?= nl2br(htmlspecialchars($row["summary"])) ?>
                        <?php else: ?>
                            <span class="text-muted">Belum disummarize</span>
                        <?php endif; ?>
                    </div>
                </td>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea class="form-control" name="summary" rows="3" 
                                                    id="summary_<?= $row["id"] ?>"><?= htmlspecialchars($row["summary"] ?? '') ?></textarea>
...
document.querySelectorAll('.btn-ai').forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        if (!isi || isi.length < 30) {
            target.innerHTML = '❌ Teks terlalu pendek'
            return
        }

        target.innerHTML = '⏳ AI lagi mikir...'

        try {
            const res = await fetch('ai/summarize.php', {
                method: 'POST',
                headers: {
            ...
            if (data.summary) {
                ...
                target.innerHTML = '❌ AI tidak menghasilkan ringkasan'
            }

        } catch (e) {
            target.innerHTML = '❌ Gagal summarize'
        }
    })
})
</script>
